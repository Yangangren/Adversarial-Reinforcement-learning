...
def traj_segment_generator(pro_pi, adv_pi, env, horizon, stochastic):
    ...
    adv_ac = env.adv_action_space.sample()
    ...
    while True:
        pro_prevac = pro_ac
        adv_prevac = adv_ac
        # 给protagonist policy 输入state，返回action and value
        pro_ac, pro_vpred = pro_pi.act(stochastic, ob)
        adv_ac, adv_vpred = adv_pi.act(stochastic, ob)
        # clip the action
        pro_ac = np.clip(pro_ac, env.action_space.low, env.action_space.high)
        adv_ac = np.clip(adv_ac, env.adv_action_space.low, env.adv_action_space.high)

        # ac.pro = pro_ac
        # ac.adv = adv_ac
        ac = np.append(pro_ac,adv_ac)
...
def evaluate(pi, adv_pi, env, rollouts=50):
    retlst = []
    for _ in range(rollouts):
        ...
        while not done:
            ...
            ac_adv = adv_pi.act(False, ob)[0]
            ac = np.append(ac, ac_adv)
...
def learn(env, test_env, policy_func, *,
        ...
        clip_param, optim_epochs, optim_stepsize, optim_batchsize,# optimization hypers
        gamma, lam, # advantage estimation
        max_timesteps=0, max_episodes=0, max_iters=0, max_seconds=0,  # time constraint
        entcoeff=0.0, vf_coef=0.5,
    ...
    pro_assign_old_eq_new = U.function([],[], updates=[tf.assign(oldv, newv) for (oldv, newv) in zipsame(pro_oldpi.get_variables(), pro_pi.get_variables())])
    adv_assign_old_eq_new = U.function([],[], updates=[tf.assign(oldv, newv) for (oldv, newv) in zipsame(adv_oldpi.get_variables(), adv_pi.get_variables())])
    ...
    for update in range(1, max_iters + 1):
        ...
        if save_interval and (update == 1 or iters_so_far % save_interval == 0) and logger.get_dir():
